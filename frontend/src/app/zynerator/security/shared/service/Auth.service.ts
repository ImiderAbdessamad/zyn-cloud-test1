...
export class AuthService {
    ...
    */
    ...
    public registerCollaborator() {
        this.register(this.registerUrl + 'collaborator', 'ROLE_COLLABORATOR', '/collaborator/activation')
    }

    public registerInfluencer() {
        this.register(this.registerUrl + 'influencer', 'ROLE_INFLUENCER', '/influencer/activation')
    }

    public registerOpen() {
        this.register(this.registerUrl + 'open', 'ROLE_OPEN', '/open/activation')
    }

    public registerAgent() {
        this.register(this.registerUrl + 'agent', 'ROLE_AGENT', '/agent/activation')
    }
            ...
            .pipe(
                map(resp => {
                    if (this.user.roleUsers[0].role.authority === 'ADMIN') {
                        ...
                        else if (this.user.roleUsers[0].role.authority === 'COLLABORATOR') {
                        this.router.navigate(['/collaborator/login']);
                        }
                        else if (this.user.roleUsers[0].role.authority === 'INFLUENCER') {
                        this.router.navigate(['/influencer/login']);
                        }
                        else if (this.user.roleUsers[0].role.authority === 'OPEN') {
                        this.router.navigate(['/open/login']);
                        }
                        else if (this.user.roleUsers[0].role.authority === 'AGENT') {

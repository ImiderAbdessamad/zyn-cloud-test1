...
export class ProjectEditAdminComponent implements OnInit {
    ...
    public prepareEdit() {
        ...
        this.item.chatDateStart = this.service.format(this.item.chatDateStart);
    }
 ...
 public edit(): void {
    ...
    public validateConversations(){
        this.errorMessages = new Array();
    }

    public validateProjectDetails(){
        this.errorMessages = new Array();
   ...
   public addConversations() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.conversationsElement.id == null){
                this.item.conversations.push(this.conversationsElement);
            }else{
                const index = this.item.conversations.findIndex(e => e.id == this.conversationsElement.id);
                this.item.conversations[index] = this.conversationsElement;
            }
   ...
   public addProjectDetails() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.projectDetailsElement.id == null){
                this.item.projectDetails.push(this.projectDetailsElement);
            }else{
                const index = this.item.projectDetails.findIndex(e => e.id == this.projectDetailsElement.id);
                this.item.projectDetails[index] = this.projectDetailsElement;
            }
    ...
    private validateProjectDetailsTitle(){
    ...
    private validateProjectDetailsProjectTechnology(){
    ...
    private validateProjectDetailsProjectTechnologyProfile(){
    ...
    private validateProjectDetailsDbName(){
    ...
    private validateProjectDetailsDbPassword(){
    ...
    private validateProjectDetailsDbUserName(){
    ...
    private validateProjectDetailsBasePackage(){
    ...
    private validateProjectDetailsMsName(){
    ...
    private validateProjectDetailsPort(){
   ...
   public async openCreateRemoteRepoInfo(remoteRepoInfo: string) {
        const isPermistted = await this.roleService.isPermitted('RemoteRepoInfo', 'edit');
        if (isPermistted) {

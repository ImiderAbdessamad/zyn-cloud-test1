...
@Component({
    selector: 'app-forget-password-agent',
    templateUrl: './forget-password-agent.component.html',
    styleUrls: ['./forget-password-agent.component.css'],
})
export class ForgetPasswordAgentComponent {
    ...
    submit() {
        this.authService.sendForgetPasswordActivation(this.forgetPasswordDto.email).subscribe({
            next: () => {
                ...
                this.router.navigate(['agent/changePassword'])
            },

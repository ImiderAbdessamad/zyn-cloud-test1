...
@SpringBootTest
public class CustumorSupportConversationDaoTest {

@Autowired
    private CustumorSupportConversationDao underTest;
    ...
    void shouldFindById(){
        ...
        CustumorSupportConversation loaded = underTest.findById(id).orElse(null);
    ...
    void shouldFindAll() {
        // Given
        List<CustumorSupportConversation> items = IntStream.rangeClosed(1, 10).mapToObj(i->constructSample(i)).collect(Collectors.toList());

        // Init
        items.forEach(underTest::save);

        // When
        List<CustumorSupportConversation> loadedItems = underTest.findAll();
    ...
    @Test
    void shouldSave(){
        CustumorSupportConversation given = constructSample(1);
        CustumorSupportConversation saved = underTest.save(given);

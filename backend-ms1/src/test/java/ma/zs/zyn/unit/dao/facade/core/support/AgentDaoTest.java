...
@Autowired
    ...
    void shouldFindById(){
        ...
        Agent loaded = underTest.findById(id).orElse(null);
    ...
    void shouldFindAll() {
        // Given
        List<Agent> items = IntStream.rangeClosed(1, 10).mapToObj(i->constructSample(i)).collect(Collectors.toList());

        // Init
        items.forEach(underTest::save);

        // When
        List<Agent> loadedItems = underTest.findAll();
    ...
    @Test
    void shouldSave(){
        Agent given = constructSample(1);
        Agent saved = underTest.save(given);
